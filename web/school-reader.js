...
export function extractSchoolDocument(doc) {
  ...
  const parentTable=node=>{for(let n=node;n;n=n.parentElement)if(n.tagName==='TABLE')return n;return null;};
  ...
  const appendCard=(card,day)=>{
    stage='course-text';
    if(unique(nodes(card.querySelectorAll('.title,.timetable_title')).map(n=>String(n.textContent||'').trim()).filter(Boolean)).length>1)throw Error('同一课程块中出现多个课程名称，已停止读取以免混淆。请复制诊断反馈或使用 PDF 导入。');
    const copy=card.cloneNode(true);
    nodes(copy.querySelectorAll('script,style,input,textarea,select,button,[hidden]')).forEach(n=>{if(n.parentNode)n.parentNode.removeChild(n);});
    for(const [selector,label] of [['.glyphicon-time','时间：'],['.glyphicon-map-marker','地点：'],['.glyphicon-user','教师：']])nodes(copy.querySelectorAll(selector)).forEach(n=>n.parentNode.replaceChild(doc.createTextNode('\n'+label),n));
    nodes(copy.querySelectorAll('br')).forEach(n=>n.parentNode.replaceChild(doc.createTextNode('\n'),n));
    nodes(copy.querySelectorAll('p,div,li,tr')).forEach(n=>{n.insertBefore(doc.createTextNode('\n'),n.firstChild);n.appendChild(doc.createTextNode('\n'));});
  ...
  for(const table of nodes(doc.querySelectorAll('table'))){
    stage='headers';
    const rows=nodes(table.rows).filter(r=>parentTable(r)===table);
    const heads=rows.find(r=>unique(nodes(r.cells).map(cellDay).filter(Boolean)).length>=5);
    if(!heads)continue;
    stage='table-grid';
    const grid=[],headers=[];
    rows.forEach((row,y)=>{grid[y]??=[];let x=0;for(const cell of nodes(row.cells)){while(grid[y][x])x++;const w=Math.min(cell.colSpan||1,20),h=Math.min(cell.rowSpan||1,100);
      ...
      if(row===heads&&cellDay(cell))for(let dx=0;dx<w;dx++)headers.push([x+dx,cellDay(cell)]);
      x+=w;
    }});
    for(let y=rows.indexOf(heads)+1;y<rows.length;y++)for(const [x,day] of headers){
      const cell=grid[y]?.[x];if(!cell||seen.indexOf(cell)!==-1||parentTable(cell)!==table)continue;seen.push(cell);
  ...
  if(!entries.length&&doc.defaultView){
    ...
    for(const node of nodes(doc.querySelectorAll('th,td,[role=columnheader],span,div'))){
      ...
      let group=groups.find(g=>Math.abs(g.y-r.top)<8);if(!group){group={y:r.top,days:[]};groups.push(group);}
      const previous=group.days.find(item=>item[0]===day);if(!previous)group.days.push([day,{left:r.left,right:r.right,width:r.width}]);else if(r.width>previous[1].width)previous[1]={left:r.left,right:r.right,width:r.width};
    }
    const candidates=groups.filter(g=>g.days.length>=5).sort((a,b)=>b.y-a.y);
  ...
  }catch(e){e.readerStage=stage;throw e;}
}

// Only a SecurityError while accessing a frame document counts as blocked.
export function readSchoolPage(root){
  const stack=[{win:root,depth:0}],visited=[],framePages=[],stats={version:'1.1.4',documents:0,blockedFrames:0,tables:0,nestedTables:0,weekdayCells:0,cards:0,maxDepth:0,frameElements:0,frameOrigins:[],readerErrors:[]},errors=[];
  const fault=(stage,e)=>{const names=['Error','TypeError','SecurityError','ReferenceError','SyntaxError','RangeError'],item={stage,type:names.includes(e?.name)?e.name:'UnknownError'};if(['headers','course-text','table-grid','column-position'].includes(e?.readerStage))item.step=e.readerStage;stats.readerErrors.push(item);};
  while(stack.length){
    const {win,depth}=stack.shift();if(visited.indexOf(win)!==-1||depth>6)continue;visited.push(win);
    let doc;

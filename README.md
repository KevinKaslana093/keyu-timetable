...
1. Android 可在“导入课表 → 华工教务直接导入”中登录学校，查询个人课表并切换表格后读取（实验功能，真实账号流程待验证）。也可导入华工 PDF、保存的教务表格 HTML 或标准七列 XLSX / CSV / HTML。
2. 核对完整预览。确认后新建课表，已有数据保留。
3. 设置校历“第 1 周周一”、选择“五山”或“大学城 / 国际校区”作息、核对学期周数。预设含完整 11 节上下课时间，也支持自定义。
...
尚未实现：无人值守自动登录、任意学校 PDF / 任意布局 Excel、图片 OCR、WakeUp 私有备份/分享口令、ICS 导入、桌面小组件、iCloud、原生 iOS / HarmonyOS 安装包。浏览器关闭后的可靠提醒使用系统日历，不依赖网页后台计时器。
...
Android 增加独立学校浏览窗口，由用户在学校统一认证页输入凭据。查询个人课表后点击“读取当前课表”，文本解析完成后仍需预览确认。远程网页没有课屿的原生数据接口。会话在关闭时清除；课表仍只保存在本机。学校登录使用 HTTPS，但学校自身返回 HTTP 教务地址；网络明文例外仅限 `xsjw2018.jw.scut.edu.cn`，不会忽略证书错误。

网页版受浏览器跨站限制，使用 PDF 或已保存的教务 HTML 导入。实验网页解析支持有星期表头及标准节次文本的表格；不保证所有教务升级布局。真实账号验证需要在学校网络环境由学生本人进行，仓库不含账号或 Cookie。

[作息来源与网页导入验证](docs/SCHOOL_IMPORT.md)。
